...
# Load CSVs
df = pd.read_csv("PHQ_preprocessed.csv")
...
for test_id in tqdm(subject_ids, desc="LOSO-CV"):
    ...
    for _, row in test_df.iterrows():
        ...
        prompt = (
            "You are a highly reliable assistant that predicts depression risk using the features."
            " Based on input features, You must output only a single digit: 0 or 1 to predict depression, with NO text, NO explanation, and NO punctuation."
        ...
        all_labels.append(row["PHQ_score"])
